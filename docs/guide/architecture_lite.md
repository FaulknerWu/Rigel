...
本页描述的是项目的目标架构，不代表仓库已经完整落地以下所有阶段。基于当前代码现状：

- `core/` 已完成 Rust crate 脚手架，以及 `config`、`graph`、`llm`、`mcp`、`model`、`parser` 模块占位
- `parser` 目前仅保留 Java AST 解析入口注释，尚未形成真正的实体抽取逻辑
- `graph`、`llm`、`mcp` 仍为占位模块，尚未具备可运行的业务实现
- `labs/` 仅保留 Python 预研依赖定义，没有持续维护的实验脚本

因此，下面的架构图应被理解为“当前目标形态”，而不是“当前代码已实现形态”。

## 核心实现机制

在目标状态下，系统内部流转中各项功能被解耦为阶段性的处理单元，整体上构成“抽取 => 增补与持久化 => 外部协议服务”的三阶段架构。
